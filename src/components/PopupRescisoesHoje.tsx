...
// Mesmo mecanismo de PopupAniversariosHoje.tsx (checagem ao carregar o painel + "marcar
// visto" upsert por dia) — só o conteúdo muda (avisos de rescisão em vez de aniversário).
export default function PopupRescisoesHoje() {
  const router = useRouter();
  const [avisos, setAvisos] = useState<AvisoRescisaoHoje[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/rescisoes/avisos-hoje");
        if (!res.ok) return;
        const body = await res.json();
        if (cancelado) return;
        const lista: AvisoRescisaoHoje[] = body.data ?? [];
  ...
  async function marcarVisto() {
    setAberto(false);
    try {
      await fetch("/api/rescisoes/avisos-hoje/marcar-visto", { method: "POST" });
    } catch {
      // se falhar, o pop-up volta a aparecer na próxima navegação — sem problema
    }
  }

  function abrirRescisao(rescisaoId: string | null) {
    marcarVisto();
    if (rescisaoId) router.push(`/painel/rescisoes?rescisao=${rescisaoId}`);
  }
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-[#FFD700]/40">
        <div className="bg-black px-6 py-5 flex items-start justify-between">
          <div>
            <h2 className="text-lg font-bold text-[#FFD700]">
              📋 Aviso{avisos.length > 1 ? "s" : ""} de rescisão
            </h2>
            <p className="text-xs text-gray-300 mt-0.5">Confira o que aconteceu hoje</p>
          </div>
          <button
            onClick={marcarVisto}
        ...
        <div className="p-6 space-y-3 max-h-[50vh] overflow-y-auto">
          {avisos.map((a) => (
            <button
              key={a.id}
              onClick={() => abrirRescisao(a.rescisao_id)}
