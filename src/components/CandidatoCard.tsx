...
interface Props {
  candidato: Candidato;
  onMover: (id: string, etapa: string, comentario?: string) => Promise<void>;
  movendo: boolean;
  matchScore?: number;
}
...
export default function CandidatoCard({ candidato, onMover, movendo, matchScore }: Props) {
  ...
  const handleSelectChange = (value: string) => {
    if (value === candidato.etapa_kanban || value === "") return;
    const opcoes = OPCOES_POR_ETAPA[candidato.etapa_kanban] ?? [];
  ...
  const handleConfirmar = async () => {
    ...
    setModalEtapa(null);
    await onMover(candidato.id, modalEtapa.value, comentario.trim() || undefined);
    setComentario("");
  };
