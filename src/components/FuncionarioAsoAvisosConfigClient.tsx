...
export default function FuncionarioAsoAvisosConfigClient({
  ...
  const handleAdicionarEmail = async (e: React.FormEvent) => {
    ...
    setErroEmail("");
    try {
      const res = await fetch("/api/funcionario-aso-avisos-config/email", {
        method: "POST",
  ...
  const handleToggleAtivo = async (id: string, ativo: boolean) => {
    ...
    try {
      const res = await fetch(`/api/funcionario-aso-avisos-config/email/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleRemoverEmail = async (id: string, nome: string) => {
    if (!confirm(`Remover "${nome}" dos avisos de ASO periódico por e-mail?`)) return;
    const anterior = emailDestinatarios;
    setEmailDestinatarios((prev) => prev.filter((d) => d.id !== id));
    try {
      const res = await fetch(`/api/funcionario-aso-avisos-config/email/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
    } catch {
  ...
  const handleTogglePlataforma = async (usuario: UsuarioOption, incluido: boolean) => {
    ...
    try {
      if (incluido) {
        const res = await fetch("/api/funcionario-aso-avisos-config/plataforma", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        if (!atual) return;
        const res = await fetch(`/api/funcionario-aso-avisos-config/plataforma/${atual.id}`, { method: "DELETE" });
  ...
  return (
    <div>
      <div className="mb-5">
        <h1 className="text-xl font-bold text-gray-900">Avisos de ASO Periódico</h1>
        <p className="text-sm text-gray-500 mt-1">
          Lista fixa de destinatários — vale automaticamente para todo aviso de ASO periódico vencendo ou vencido, sem precisar escolher a cada vez.
        </p>
      </div>
        ...
        >
          {semEmailAtivo && (
            <p style={{ margin: 0, fontSize: 13, color: "#92400E", fontWeight: 600 }}>
              ⚠️ Nenhum destinatário de e-mail ativo — os avisos de ASO periódico não serão enviados por e-mail para ninguém até que alguém seja adicionado ou reativado.
            </p>
          )}
          {semPlataforma && (
