...
// ── chip data (mirrored from ModalNovaVaga) ───────────────────────────────────
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-black text-white px-6 py-4 rounded-t-2xl flex items-center justify-between">
          <h2 className="font-bold text-lg">Editar vaga</h2>
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <input value={form.titulo} onChange={(e) => set("titulo", e.target.value)}
              placeholder="Ex: Analista de RH Pleno" className="input-field" />
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input type="number" min="1" value={form.num_posicoes}
                onChange={(e) => set("num_posicoes", e.target.value)} placeholder="1" className="input-field" />
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.cidade} onChange={(e) => set("cidade", e.target.value)}
                placeholder="Ex: São Paulo" className="input-field" />
            </div>
            <div>
          ...
          <div className="border-t pt-4">
            ...
            {horTipo && (
              <div className="flex items-center gap-2 mb-3">
                ...
                <input value={horInicio} onChange={(e) => setHorInicio(maskHora(e.target.value))}
                  placeholder="08h00" maxLength={5} className="input-field w-24" />
                <span className="text-sm text-gray-500 shrink-0">às</span>
                <input value={horFim} onChange={(e) => setHorFim(maskHora(e.target.value))}
                  placeholder="17h30" maxLength={5} className="input-field w-24" />
              </div>
            )}
            <input value={horOutros} onChange={(e) => setHorOutros(e.target.value)}
          ...
          <div className="border-t pt-4">
                  ...
                  <div key={chip} className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {ativo && REQUISITOS_COM_TEXTO.has(chip) && (
                      <div className="flex items-center gap-1">
                        ...
                        <input value={reqTexto[chip] ?? ""}
                          onChange={(e) => setReqTexto((t) => ({ ...t, [chip]: e.target.value }))}
                          placeholder="ex: Administração" className="input-field text-xs h-7 py-1 w-36" />
                      </div>
                    )}
                  </div>
                );
            ...
            {reqCustom.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-3">
                {reqCustom.map((chip) => (
                  <span key={chip} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full font-medium"
                    style={{ backgroundColor: "#000000", color: "#ffffff" }}>
                    {chip}
                    <button type="button" onClick={() => setReqCustom((prev) => prev.filter((c) => c !== chip))}
            ...
            <div className="flex gap-2">
              <input value={reqInput} onChange={(e) => setReqInput(e.target.value)}
                ...
                placeholder="Ex: CRQ ativo, MOPP, NR35..." className="input-field text-sm flex-1" />
              <button type="button" onClick={adicionarReqCustom} className="btn-outline text-sm shrink-0">Adicionar</button>
            </div>
          </div>
          ...
          <div className="border-t pt-4">
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {benCustom.map((chip) => (
                  <span key={chip} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full font-medium"
                    style={{ backgroundColor: "#000000", color: "#ffffff" }}>
                    {chip}
                    <button type="button" onClick={() => setBenCustom((prev) => prev.filter((c) => c !== chip))}
                      className="opacity-60 hover:opacity-100 leading-none">×</button>
                  </span>
                ))}
              </div>
            ...
            <div className="flex gap-2">
              <input value={benInput} onChange={(e) => setBenInput(e.target.value)}
                ...
                placeholder="Ex: Auxílio creche, Psicologia Viva..." className="input-field text-sm flex-1" />
              <button type="button" onClick={adicionarBenCustom} className="btn-outline text-sm shrink-0">Adicionar</button>
            </div>
          </div>
          ...
          <div className="border-t pt-4">
            ...
            <textarea value={form.observacoes} onChange={(e) => set("observacoes", e.target.value)}
              placeholder="Anotações internas sobre a vaga..." rows={2} className="input-field resize-none" />
          </div>
