...
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    const { resultado, data_inicio, motivo_reprovacao, observacoes } = body;
    ...
    const { data: cv, error: cvErr } = await supabase
      ...
      .select("id, candidato_id, vaga_id, vagas(id, titulo, num_posicoes_abertas)")
      .eq("id", id)
      .single();
    ...
    const vaga = (cv as any).vagas as { id: string; titulo: string; num_posicoes_abertas: number | null } | null;
    ...
    await supabase
      .from("candidatos_vagas")
      .update({ etapa: "reprovado_final", motivo_reprovacao, observacoes: observacoes || null })
      .eq("id", id);
    ...
    void registrarHistorico({
      ...
      descricao: `Processo encerrado para ${vagaTitulo}. Motivo: ${motivo_reprovacao}.${observacoes ? ` ${observacoes}` : ""}`,
      metadata: { cv_id: id, vaga_id: cv.vaga_id, motivo_reprovacao, observacoes },
    });

    return NextResponse.json({ resultado: "reprovado_final" });
  } catch (err) {
