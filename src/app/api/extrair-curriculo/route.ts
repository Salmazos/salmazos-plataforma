...
export async function POST(req: NextRequest) {
  try {
    ...
    // Normalize extracted phone to 11 digits (DDD + 9 + 8 digits)
    if (extraido.telefone != null) {
      let digits = String(extraido.telefone).replace(/\D/g, "");
      if (digits.length === 10) {
        digits = digits.slice(0, 2) + "9" + digits.slice(2);
      }
      extraido.telefone = digits.length === 11 ? digits : null;
    }
    console.log("TELEFONE EXTRAIDO:", extraido.telefone);
