...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  const parsed = parseBody(rescisaoAvisoEmailUpdateSchema, body);
  ...
  // Desativar sem remover é a mesma falha silenciosa que remover: a pessoa para de
  // receber aviso de pagamento pendente sem ninguém perceber — vale o mesmo registro.
  registrarAuditoria({
    usuario_id: user.id,
    usuario_nome: user.email ?? null,
    acao: data.ativo ? "rescisao_aviso_email_reativado" : "rescisao_aviso_email_desativado",
...
export async function DELETE(_request: NextRequest, { params }: Params) {
  ...
  if (data) {
    registrarAuditoria({
      usuario_id: user.id,
      usuario_nome: user.email ?? null,
      acao: "rescisao_aviso_email_removido",
      entidade: "rescisao_avisos_email_destinatarios",
      entidade_id: data.id,
