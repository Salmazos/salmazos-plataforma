...
export default async function FuncionarioAsoAvisosConfigPage() {
  ...
  const [{ data: emailDestinatarios }, { data: plataformaDestinatariosRaw }, { data: usuarios }] = await Promise.all([
    svc.from("funcionario_aso_avisos_email_destinatarios").select("*").order("nome"),
    svc.from("funcionario_aso_avisos_plataforma_destinatarios").select("*").order("criado_em"),
    svc.from("analistas_perfil").select("user_id, nome_completo, email").eq("ativo", true).order("nome_completo"),
  ]);

  // Sem FK direta entre funcionario_aso_avisos_plataforma_destinatarios e analistas_perfil
  // (as duas só referenciam auth.users separadamente) — resolve o nome aqui em vez de um
